...
public class ListPanel extends JPanel implements ActionListener {
    ...
    private JButton addMarketB = new JButton("add a market");
    private JCheckBox inputCB = new JCheckBox("");
    private JTextField inputForm = new JTextField("", 4);

    private RestaurantBayou restPanel;
    private String type;
     ...
     */
    public ListPanel(RestaurantBayou rp, String type) {
        restPanel = rp;
        this.type = type;
        ...
        if (type.equals("markets")){
        	addMarketB.addActionListener(this);
        	inputLayout.add(addMarketB);
        	inputCB.setText("empty?");
        }
...
//            addPerson(JOptionPane.showInputDialog("Please enter a name:"));
        	...
        	inputCB.setSelected(false);
        } else if (e.getSource() == addMarketB) {
        	addMarket(inputCB.isSelected());
        	inputCB.setSelected(false);
        }
        else {
...
//          JLabel label = new JLabel("Hungry?");
    	  ...
    	  String name = restPanel.addWaiter(onBreak);
          Dimension paneSize = pane.getSize();
    ...
    public void addMarket(Boolean empty){
//      JLabel label = new JLabel("Hungry?");

	  String name = restPanel.addMarket(empty);
      Dimension paneSize = pane.getSize();
      ...
      stateCB.setHorizontalTextPosition(SwingConstants.LEFT);
      stateCB.setVisible(true);
      stateCB.setSelected(empty);
      stateCB.setEnabled(!empty);
      stateCB.addActionListener(this);
      list.add(stateCB);
      
      JPanel marketItem = new JPanel();
      marketItem.setLayout(new FlowLayout());
      marketItem.add(stateCB);
      marketItem.add(emptyLabel);
      
      Dimension personItemSize = new Dimension((int) paneSize.width,
              (int) (paneSize.height / 8));
      marketItem.setPreferredSize(personItemSize);
      marketItem.setMinimumSize(personItemSize);
      marketItem.setMaximumSize(personItemSize);
	...
	//                restPanel.showInfo(type, temp.getText());
    	...
    	if (person instanceof WaiterAgent) {
    		WaiterAgent wait = (WaiterAgent) person;
    		for (JCheckBox temp:list){
	            if (wait.getName() == temp.getText()) {
	                temp.setSelected(false);
	            }
	        }
    	}
